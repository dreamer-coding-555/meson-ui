...
class IntroBuildOptionsTab:
    ...
    def update_introspection(self, meson_api):
        if self._context.buildoptions_list.count() != 0:
            self._context.buildoptions_list.clear()
        self._context._model.model_options().set_list(meson_api)
